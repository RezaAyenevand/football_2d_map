...
def calculateDifference(Image):
    ...
    ## Storing The Players Count And Positions and Boundaries In Lists
    players = []
    for i in range(n):
    ...
    for i in range(n):
            if stats[i][1] > 100 and stats[i][3] > 1.1 * stats[i][2] and stats[i][4] > 65 :
                players_pos.append([centroids[i][0],int(centroids[i][1] + stats[i][3]/2)])
    players_bound = []
    for i in range(n):
            if stats[i][1] > 100 and stats[i][3] > 1.1 * stats[i][2] and stats[i][4] > 65 :
                players_bound.append([stats[i][0],stats[i][1],stats[i][0] + stats[i][2],stats[i][1] + stats[i][3]])
    ...
    ## Defining An Image That Only Has Circles On Players Locations (This Will Later Be Transformed Using Homography)
    ...
    for i in range(len(players_pos)):
        ...
        if y_test[i] == 0:
            ...
            cv2.rectangle(Image, (player_bound[0], player_bound[1]), (player_bound[2], player_bound[3]), (0, 0, 255), 1)
        elif y_test[i] == 1:
            cv2.circle(locImageBlue, (int(player_pos[0]), int(player_pos[1])), 5, (255, 0, 0), 5)
            cv2.rectangle(Image, (player_bound[0], player_bound[1]), (player_bound[2], player_bound[3]), (255, 0, 0), 1)
        elif y_test[i] == 2:
            cv2.circle(locImageWhite, (int(player_pos[0]), int(player_pos[1])), 5, (255, 255, 255), 5)
            cv2.rectangle(Image, (player_bound[0], player_bound[1]), (player_bound[2], player_bound[3]), (0, 255, 255), 1)
        else:
            pass
...
parser.add_argument('--algo', type=str, default='KNN')
...
out = cv2.VideoWriter('detected.avi', fourcc, 30.0, (1280, 960))
...
while True:
    ...
    if p < 16 :
        ...
        p = 0
    dotsImage = []
    dotsImage = calculateDifference(I)
